// src/public/js/agenda.medico.js
(function () {
  const { doctorId } = (window.MEDICO || {});
  if (!doctorId) console.warn('DoctorId ausente para agenda do médico.');
  ...
  if (!$modalRoot) {
    ...
    $modalRoot.innerHTML = `<div class="modal-backdrop" data-close></div><div class="modal-slot"></div>`;
  ...
  async function loadMonthData(){
    ...
    const qs = new URLSearchParams({ from: first.toISOString(), to: last.toISOString() }).toString();

    const r = await fetch(`/agenda-medico/data?${qs}`);
  ...
  function setSelected(d){
    ...
    document.querySelectorAll('.day')
      .forEach(el => el.classList.toggle('is-selected', el.dataset.key === toKey(selectedDate)));
  }
  ...
  function renderPainel(d){
    ...
    function renderList(list, target, kind){
      ...
      target.innerHTML = list.map(item => {
        ...
        const who = item.patient?.name ? item.patient.name : 'Horário disponível';
  ...
  function openDetail(type, id){
    ...
    if (type === 'AVAILABLE') {
      ...
      html = `
        <div class="detail-list">
          ...
          <div class="row"><div class="label">Status</div><div class="value"><strong>${appt.status}</strong></div></div>
        </div>
        <div class="modal__footer">
          <button type="button" class="btn btn--ghost" data-close>Voltar</button>
    ...
    if (btn) btn.addEventListener('click', () => confirmPending(btn.getAttribute('data-confirm-appt'), btn));
  }
  ...
  function closeModal(){ $modalRoot.classList.remove('is-open'); $modalSlot.innerHTML = ''; }

  // Dropdown Disponibilizar
  function toggleMenu(show){ $menuDisp.setAttribute('aria-hidden', show ? 'false' : 'true'); }
  toggleMenu(false);
  $btnDisp.addEventListener('click', (e) => {
    e.stopPropagation();
    toggleMenu($menuDisp.getAttribute('aria-hidden') !== 'false');
  });
  ...
  $menuDisp.addEventListener('click', (e) => {
    ...
    if (!el) return;
    toggleMenu(false);
    if (el.dataset.action === 'especifica') openDisponibilizarEspecifica();
    if (el.dataset.action === 'lote')       openDisponibilizarLote();
  });
  ...
  function openDisponibilizarEspecifica(){
    ...
    $form.addEventListener('submit', async (ev) => {
      ...
      try {
        await fetchJson('/agenda-medico/availability', 'POST', payload);
  ...
  function openDisponibilizarLote(){
    ...
    $form.addEventListener('submit', async (ev) => {
      ...
      try {
        const resp = await fetchJson('/agenda-medico/availability/day-openings', 'POST', payload);
  ...
  async function confirmPending(id, btn){
    try {
      ...
      await fetch(`/agenda-medico/appointments/${id}/confirm`, { method: 'POST', headers: { 'Accept': 'application/json' } });
      const first = new Date(viewYear, viewMonth, 1);
